 ...
 * PeerJS is used only for rendezvous/control. Media is direct LAN-first with no STUN/TURN.
 * One host + one client per room, host heartbeat, two-way media, real receiver telemetry.
 */
class WebRTCManager {
    constructor(options = {}) {
    ...
    _createClientPeer() {
        ...
        peer.on('error', err => {
            ...
            if (err?.type === 'peer-unavailable' || err?.type === 'unavailable-id') this._setStatus('ROOM_NOT_FOUND', 'Phòng không tồn tại hoặc đã đóng');
            else this._setStatus('FAILED', 'Không thể tham gia phòng');
        });
    ...
    _acceptClient(conn) {
        ...
        conn.on('data', data => { if (data?.type === 'client-hello') this._setStatus('CONNECTED', 'Đã kết nối thiết bị thứ hai'); });
    ...
    _handleIncomingCall(call) {
        if (this.closed) { try { call.close(); } catch (_) {} return; }
    ...
    async startScreenCapture(stream) {
        ...
        for (const track of stream.getTracks()) {
            ...
            track.addEventListener('ended', () => { if (this.localStream === stream) this.stopScreenCapture(true); }, { once: true });
        }
        if (!this.hasPeer) { this._setStatus('WAITING', 'Đang chờ thiết bị thứ hai…'); return; }
    ...
    _startStats() {
        ...
        this.statsTimer = setInterval(async () => {
            try {
                ...
                let fps = 0, rtt = 0, jitter = 0, decode = 0, localType = '', remoteType = '', bytes = 0, packetsLost = 0, packetsReceived = 0;
                for (const call of calls) {
                    ...
                    stats.forEach(report => {
                        if (report.type === 'inbound-rtp' && (report.kind === 'video' || report.mediaType === 'video')) {
                            ...
                            if (Number.isFinite(report.bytesReceived)) bytes = Math.max(bytes, report.bytesReceived);
                            if (Number.isFinite(report.packetsLost)) packetsLost = Math.max(packetsLost, report.packetsLost);
                            if (Number.isFinite(report.packetsReceived)) packetsReceived = Math.max(packetsReceived, report.packetsReceived);
                        }
                ...
                if (this.lastBytesAt && now > this.lastBytesAt && bytes >= this.lastBytes) mbps = ((bytes - this.lastBytes) * 8 / ((now - this.lastBytesAt) / 1000)) / 1e6;
                ...
                if (this.onMetrics) this.onMetrics({
                    fps: Math.round(fps || 0), rtt: Math.round(rtt || 0), ping: Math.round(rtt || 0), bitrateMbps: mbps, lossPct: loss,
                    pipelineMs: Math.round(Math.max(0, rtt + jitter + decode)), localCandidateType: localType, remoteCandidateType: remoteType,
    ...
    _destroyPeerOnly() {
        clearInterval(this.heartbeatTimer); clearInterval(this.clientWatchdog);
        this.heartbeatTimer = null; this.clientWatchdog = null;
