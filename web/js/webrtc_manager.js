...
class WebRTCManager {
    constructor(options = {}) {
        ...
        this.outgoingCall = null; this.incomingCalls = new Set();
        this.localStream = null; this.remoteStreams = new Map();
        this.statsTimer = null; this.heartbeatTimer = null; this.clientWatchdog = null;
    ...
    _createHostPeer() {
        ...
        const peer = new Peer(this.hostPeerId,{config:this.rtcConfig,debug:0}); this.peer=peer; this.peerId=this.hostPeerId;
    ...
    _createClientPeer() {
        this._destroyPeerOnly();
        const id=`castscreen-client-${Math.random().toString(36).slice(2,11)}`; const peer=new Peer(id,{config:this.rtcConfig,debug:0}); this.peer=peer; this.peerId=id;
    ...
    _connectToHost() {
        ...
        const conn=this.peer.connect(this.hostPeerId,{reliable:true,serialization:'json'}); this.control=conn; let opened=false;
    ...
    _handleIncomingCall(call){
        if(this.closed){try{call.close();}catch(_){}return;} this.remotePeerId=call.peer; this.incomingCalls.add(call);
        try{call.answer(this.localStream||undefined);}catch(_){try{call.answer();}catch(__){}}
        call.on('stream',s=>{this.remoteStreams.set(call,s);if(this.onStream)this.onStream(s,s.getVideoTracks()[0]||null);this._startStats();});
        const cleanup=()=>{this.incomingCalls.delete(call);this.remoteStreams.delete(call);if(this.onStreamEnded)this.onStreamEnded();}; call.on('close',cleanup);call.on('error',cleanup);
    }
    async startScreenCapture(stream){
        if(!stream)throw new Error('Không có luồng màn hình'); this.stopScreenCapture(false); this.localStream=stream;
        for(const track of stream.getTracks()){if(track.kind==='video')track.contentHint='motion';track.addEventListener('ended',()=>{if(this.localStream===stream)this.stopScreenCapture(true);},{once:true});}
        if(!this.hasPeer){this._setStatus('WAITING','Đang chờ thiết bị thứ hai…');return;} this._makeOutgoingCall();
    }
    stopScreenCapture(updateStatus=true){
        if(this.localStream){for(const t of this.localStream.getTracks()){try{t.stop();}catch(_){} } this.localStream=null;}
    ...
    _startStats(){
        ...
        this.statsTimer=setInterval(async()=>{
            try{
                const calls=[this.outgoingCall,...this.incomingCalls].filter(Boolean); if(!calls.length)return;
                let fps=0,rtt=0,jitter=0,decode=0,localType='',remoteType='',bitrateBps=0,packetsLost=0,packetsReceived=0;
                for(const call of calls){
                    if(!call.peerConnection)continue; const stats=await call.peerConnection.getStats();
                    stats.forEach(r=>{
                        if(r.type==='inbound-rtp'&&(r.kind==='video'||r.mediaType==='video')){
                            if(Number.isFinite(r.framesPerSecond))fps=Math.max(fps,r.framesPerSecond);
                            if(Number.isFinite(r.jitter))jitter=Math.max(jitter,r.jitter*1000);
                            if(Number.isFinite(r.totalDecodeTime)&&r.framesDecoded>0)decode=Math.max(decode,r.totalDecodeTime/r.framesDecoded*1000);
                            const now=performance.now(); const bytes=Number(r.bytesReceived)||0; const prev=this.statsPrev.get(r.ssrc); if(prev){const dt=(now-prev.t)/1000;if(dt>0&&bytes>=prev.bytes)bitrateBps=Math.max(bitrateBps,(bytes-prev.bytes)*8/dt);} this.statsPrev.set(r.ssrc,{bytes,t:now});
                            packetsLost+=Number(r.packetsLost)||0; packetsReceived+=Number(r.packetsReceived)||0;
                        }
                        if(r.type==='candidate-pair'&&r.state==='succeeded'){
                            if(Number.isFinite(r.currentRoundTripTime))rtt=Math.max(rtt,r.currentRoundTripTime*1000);
                            if(r.localCandidateId){const c=stats.get(r.localCandidateId);if(c?.candidateType)localType=c.candidateType;}
                            if(r.remoteCandidateId){const c=stats.get(r.remoteCandidateId);if(c?.candidateType)remoteType=c.candidateType;}
                        }
                    });
                }
                if(this.onMetrics)this.onMetrics({fps:Math.round(fps||0),rtt:Math.round(rtt||0),ping:Math.round(rtt||0),bandwidthBps:Math.round(bitrateBps||0),bandwidthMbps:Number((bitrateBps/1e6).toFixed(2)),packetLoss:packetsReceived+packetsLost>0?Number((packetsLost/(packetsReceived+packetsLost)*100).toFixed(2)):0,pipelineMs:Math.round(Math.max(0,rtt+jitter+decode)),localCandidateType:localType,remoteCandidateType:remoteType,lan:localType==='host'&&remoteType==='host'});
            }catch(_){ }
        },500);
    }
    ...
    close(){if(this.closed)return;this.closed=true;this._destroyPeerOnly();this.stopScreenCapture(false);clearInterval(this.statsTimer);this.statsTimer=null;this.roomReady=false;this.clientConnected=false;this.statsPrev.clear();}
}
